...
class ShardedDataIterator(object):
    ...
    def __init__(self, data: list, shard_id: int = 0, num_shards: int = 1, batch_size: int = 1, shuffle=True,
                 shuffle_seed: int = 0, offset: int = 0, strict_batch_size: bool = False):

        total_size = len(data)

        if strict_batch_size:
            total_size = math.ceil(total_size / num_shards / batch_size) * batch_size * num_shards
        else:
            total_size = math.ceil(total_size / num_shards) * num_shards
        
        rand_shuffle = random.Random(shuffle_seed)
        
        if total_size < len(data):
            self.data = rand_shuffle.sample(data, total_size)
        elif total_size > len(data):
            self.data = data + rand_shuffle.sample(data, total_size - len(data))
        else:
            self.data = data

        assert len(self.data) % num_shards == 0

        self.shards_num = max(num_shards, 1)
        self.shard_id = max(shard_id, 0)

        samples_per_shard = total_size // self.shards_num
        self.shard_start_idx = self.shard_id * samples_per_shard
        self.shard_end_idx = self.shard_start_idx + samples_per_shard

        logger.debug(
            'samples_per_shard=%d, shard_start_idx=%d, shard_end_idx=%d', 
            samples_per_shard,
...
class MultiSetDataIterator(object):
    ...
    def __init__(
        ...
        self.max_iterations = sum([iterator.max_iterations for iterator in iterator_list])
    
    def iterate_data(self, epoch: int = 0) -> Iterator[List]:
        
        items = list()
        for iterator in self.iterator_list:
            for item in iterator.iterate_data(epoch):
                items.append(item)
        
        if self.shuffle:
            epoch_rnd = random.Random(self.shuffle_seed + epoch)
            epoch_rnd.shuffle(items)
        
        for i in range(self.iteration, len(items)):
            self.iteration = self.iteration + 1
            yield items[i]
    ...
    def total_data_len(self) -> int:
        return sum([iterator.total_data_len() for iterator in self.iterator_list])
